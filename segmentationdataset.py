...
def color(data,obj):
    N = comp
    HSV = [(float(x)/N, 0.9, 1) for x in range(1,N+1)]
...
for tries in range(1):
    ...
    df = pd.read_csv("segmentation1.data",header=None)
    num = np.array(df.iloc[:,17:19])
    print("size of data",num.size)
    ...
    clf1=k_means();
    t2=time.time();
    likek += np.exp(clf1.lower_bound_)
  ...
  #  plt.plot(x,np.exp(clf1.score_samples(x)),color='r');
    ...
    clf2=rand_method()
    t2=time.time();
    likerand+=np.exp(clf2.lower_bound_)
    '''
  ...
  #  plt.plot(x,np.exp(clf2.score_samples(x)),color='k')
    ...
    clf3=greedy_method();
    t2=time.time();
    likegreed+=np.exp(clf3.lower_bound_)
    '''
...
print("K-means count: ",likek/25);
print("Rand count: ",likerand/25);
print("Greedy count: ",likegreed/25);
